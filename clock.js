var Elm = Elm || { Native: {} };Elm.Native.Basics = {};
...
Elm.Clock.make = function (_elm) {
   ...
   $Time = Elm.Time.make(_elm);
   var _op = {};
   var dateToMonthInt = function (date) {    return $Date$Core.monthToInt($Date.month(date));};
   var dateToDayInt = function (date) {    return $Date$Core.isoDayOfWeek($Date.dayOfWeek(date));};
   var arc = function (_p0) {
      var _p1 = _p0;
      return A2($List.map,function (x) {    return $Basics.fromPolar({ctor: "_Tuple2",_0: _p1._0,_1: $Basics.degrees(x)});},_U.range(_p1._1,_p1._2));
   };
   var lineStyle = _U.update($Graphics$Collage.defaultLine,{width: 7,cap: $Graphics$Collage.Flat});
   var shell = F4(function (time,numTicks,timeFn,radius) {
      var date = $Date.fromTime(time);
      var timeUnit = $Basics.toFloat(timeFn(date));
      var slice = 360 / numTicks;
      var startAng = 90 - slice * timeUnit + 0.5 * slice;
      var endAng = startAng + slice * (numTicks - 1);
      var pts = _U.list([{ctor: "_Tuple3",_0: radius,_1: startAng,_2: endAng}]);
      return $Graphics$Collage.group(A2($List.map,function (_p2) {    return A2($Graphics$Collage.traced,lineStyle,$Graphics$Collage.path(arc(_p2)));},pts));
   });
   var clock = function (t) {
      return A3($Graphics$Collage.collage,
      750,
      750,
      _U.list([A4(shell,t,60,$Date.second,100)
              ,A4(shell,t,60,$Date.minute,90)
              ,A4(shell,t,24,$Date.hour,80)
              ,A4(shell,t,7,dateToDayInt,70)
              ,A4(shell,t,12,dateToMonthInt,60)
              ,$Graphics$Collage.text($Text.monospace($Text.fromString("hm / 2016")))]));
   };
   var main = A2($Signal.map,clock,$Time.every($Time.second));
